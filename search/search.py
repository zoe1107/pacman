...
def breadthFirstSearch(problem):
    ...
    while not myQueue.isEmpty():
        ...
        if problem.isGoalState(front[0]):
            return front[1]
        successors = problem.getSuccessors(front[0])
        for p in successors:
            if p[0] not in visited:
                visited.add(p[0])
                myQueue.push((p[0], front[1]+[p[1]]))
    return None   
    ...
    while not myPQ.isEmpty():
        ...
        if front[0] not in visited:
            ...
            for p in successors:
                if p[0] not in visited:
                    cost = front[2] + p[2]
                    myPQ.push((p[0], front[1]+[p[1]],cost),cost)
    return None   
    util.raiseNotDefined()
    ...
    while not myPQ.isEmpty():
        ...
        if front[0] not in visited:
            ...
            for p in successors:
                if p[0] not in visited:
                    cost = front[2] + p[2]
                    fx = heuristic(p[0], problem) + cost
                    myPQ.push((p[0], front[1]+[p[1]],cost),fx)
    return None   
    util.raiseNotDefined()
